...
export async function createHeroScene(opts: HeroSceneOptions): Promise<HeroScene> {
  ...
  const fitDistance = () => {
    ...
    const byHeight = 1.7 / tan;
    const byWidth = 3.2 / (tan * camera.aspect);
  ...
  const renderFrame = () => {
    ...
    // Pose final: 3/4 frontal, levemente elevada.
    const restAz = THREE.MathUtils.degToRad(-32);
    ...
    if (car && !identified) {
      ...
      const p = ease(t);
      const x = lerp(-CAR_LENGTH / 2 - 0.2, CAR_LENGTH / 2 + 0.2, p);
      clip.constant = x + 0.06;
      bar.position.x = x;
      (bar.material as { opacity: number }).opacity = Math.sin(Math.min(1, t) * Math.PI) * 0.9 + 0.1;
      placeCamera(lerp(THREE.MathUtils.degToRad(-70), restAz, p), lerp(0.03, restEl, p), dist * lerp(0.82, 1, p));
      if (t >= 1) {
        identified = true;
        revealDone();
        opts.onIdentified();
